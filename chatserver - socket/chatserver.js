...
http.listen(7099, function () {
	console.log('Listening on 7099');
});
...
io.on('connection', function (socket) {
	...
	socket.on('kick', function (kickObj, fn) {
		...
		if(rooms[kickObj.room].ops[socket.username] !== undefined) {
			const opsInRoom = rooms[kickObj.room].ops;
			...
			io.sockets.emit('updateusers', kickObj.room, rooms[kickObj.room].users, opsInRoom);
			fn(true);
		}
		else {
